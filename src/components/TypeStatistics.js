...
function TypeStatistics(props) {
    ...
    useEffect(() => {
        if (props.type) {
            ...
            .then(res => {
                //console.log(res)
                setDoubleDmgFrmTypes(res.data.damage_relations.double_damage_from)
                setHalfDmgFrmTypes(res.data.damage_relations.half_damage_from)
    ...
    return (
        <div className="type-statistics">
            {
                doubleDmgFrmTypes.map(
                    (type, index) => <Card key={index} type={type} power={"double"} />
                )
            }
            {
                halfDmgFrmTypes.map(
                    (type, index) => <Card key={index} type={type} power={"half"} />
                )
            }
            {
                noDmgFrmTypes.map(
                    (type, index) => <Card key={index} type={type} power={"any"} />
                )
            }
        </div>
    )
